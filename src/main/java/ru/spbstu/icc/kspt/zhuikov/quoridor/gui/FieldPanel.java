...
public class FieldPanel extends JPanel {
    ...
    FieldPanel(Quoridor quoridorGame) {
...
//            g.drawLine(0, i * (cellSize + spaceSize) - spaceSize, 350, i * (cellSize + spaceSize) - spaceSize);
        ...
        if (pickedMarker) {
            ...
            g2.drawRect(pickedMarkerCoordinates.getVertical() * (cellSize + spaceSize) / 2,
                    pickedMarkerCoordinates.getHorizontal() * (cellSize + spaceSize) / 2, cellSize, cellSize);
        }
    }
    ...
    private Coordinates convertCoordinates(int x, int y) {
        ...
        if (x % (cellSize + spaceSize) > cellSize) {
            fieldVertical = x / (cellSize + spaceSize) * 2 + 1;
        } else {
            fieldVertical = x / (cellSize + spaceSize) * 2;
        }

        if (y % (cellSize + spaceSize) > cellSize) {
            fieldHorizontal = y / (cellSize + spaceSize) * 2 + 1;
        } else {
            fieldHorizontal = y / (cellSize + spaceSize) * 2;
        }

        return new Coordinates(fieldHorizontal, fieldVertical);
    }
    ...
    private void pickMarker(Cell marker) {
        ...
        pickedMarkerCoordinates = new Coordinates(marker.getHorizontal(), marker.getVertical());
        repaint();
    }
