...
public class FieldTest {
    ...
    public void testPathBetweenCells1() {  // пустое поле
        ...
        QuoridorField field = new QuoridorField(9);
        assertEquals(false, field.getPathToRow(new Coordinates(16, 8), 0).empty());
        assertEquals(false, field.getPathToRow(new Coordinates(0, 8), 10).empty());
    }
    ...
    public void testPathBetweenCells2() throws FieldItemException { // "перегородка" по вертикали
        ...
        assertEquals(false, field.getPathToRow(new Coordinates(16, 8), 0).empty());
        assertEquals(false, field.getPathToRow(new Coordinates(0, 16), 0).empty());
        assertEquals(false, field.getPathToRow(new Coordinates(0, 16), 16).empty());
    }
    ...
    public void testPathBetweenCells3() throws FieldItemException { // "перегородка" по горизонтали
        ...
        assertEquals(false, field.getPathToRow(new Coordinates(0, 4), 2).empty());
        assertEquals(true, field.getPathToRow(new Coordinates(0, 4), 10).empty());
        assertEquals(true, field.getPathToRow(new Coordinates(16, 6), 0).empty());
    }
    ...
    public void testPathBetweenCells4() throws FieldItemException { // закрытая фишка
        ...
        assertEquals(true, field.getPathToRow(new Coordinates(16, 8), 6).empty());
        assertEquals(false, field.getPathToRow(new Coordinates(16, 8), 16).empty());
        assertEquals(false, field.getPathToRow(new Coordinates(0, 8), 16).empty());
    }
